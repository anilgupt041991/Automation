...
#   AMI_OWNERID_CHECK
# Description:
#   Checks whether EC2 instances are using authorized AMI owned by a specific owner ID.
#
# Rationale:
#   EC2 instances should not use public or non-authorized AMI in a business controlled environment.
#
# Indicative Severity:
#   High
#
# Trigger:
#   Periodic checks against AWS::EC2::Instance
#
# Reports on:
#   AWS::EC2::Instance
#
...
#        And: It contains a parameter key other than VpcId and OwnerId
#       Then: Return ERROR to specify valid keys
#   Scenario: 2
#      Given: Rules parameters are provided correctly. VpcId and OwnerId given.
#        And: There is no value specified for VpcId and OwnerId or VpcId does not match any VPC IDs.
#       Then: Return NO RESULTS
#   Scenario: 3
#      Given: Rules parameters are provided correctly. VpcId and OwnerId given.
#        And: Multiple VPC IDs or a single vpc id value specified for VpcId and OwnerId specified.
#       Then: Return COMPLIANT for EC2 instances whose AMI match Owner ID specified.
#   Scenario: 4
#      Given: Rules parameters are provided correctly. VpcId and OwnerId given.
#        And: Multiple VPC IDs or a single vpc id value specified for VpcId and OwnerId specified.
#       Then: Return NON_COMPLIANT for EC2 instances whose AMI does not match Owner ID specified.
#
import json
import sys
import datetime
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    if valid_rule_parameters:
        vpcs = valid_rule_parameters['VpcId'].split(",")
    for vpc in vpcs:
        response = ec2.describe_instances(Filters=[{"Name": "vpc-id", "Values": [vpc.strip()]}])
        reservations = response['Reservations']
        if reservations:
            for reserve in reservations:
                instances = reserve['Instances']
                for instance in instances:
                    ec2instances.append((instance['InstanceId'], instance['ImageId']))
                    image_ids.append(instance['ImageId'])
        else:
            print(f"No EC2 instances found in this {vpc}")
    if image_ids:
        response2 = ec2.describe_images(ImageIds=list(set(image_ids)))
        images = response2['Images']
        for ec2instance in ec2instances:
            for image in images:
                if image['OwnerId'] == valid_rule_parameters['OwnerId'] and image['ImageId'] in ec2instance:
                    evaluations.append(build_evaluation(ec2instance[0], 'COMPLIANT', event, annotation='ImageId:'+ec2instance[1]+' belongs to OwnerId:'+image['OwnerId']))
                elif image['OwnerId'] != valid_rule_parameters['OwnerId'] and image['ImageId'] in ec2instance:
                    evaluations.append(build_evaluation(ec2instance[0], 'NON_COMPLIANT', event, annotation='ImageId:'+ec2instance[1]+' belongs to OwnerId:'+image['OwnerId']))
    return evaluations

def evaluate_parameters(rule_parameters):
    valid_rule_parameters = []
    try:
        if rule_parameters['VpcId'] != "" and isinstance(rule_parameters['VpcId'], str):
            if rule_parameters['OwnerId'] != "" and isinstance(rule_parameters['OwnerId'], str):
                valid_rule_parameters = rule_parameters
            else:
                print("Please specify a valid OwnerId.")
        else:
            print("Please specify a valid VpcId.")
        return valid_rule_parameters
    except LookupError:
        print("Please input VpcId and OwnerId as the keys.")
