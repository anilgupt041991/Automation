...
#
# Rationale:
#   Regular checks to ensure that WAFV2 WEBACL has logging enabled.
#
# Indicative Severity:
#   High
#
# Trigger:
#   Periodic checks on AWS::WAFv2::WebACL
#
# Reports on:
#   AWS::WAFv2::WebACL
#
# Rule Parameters:
#   Scope
#     (Mandatory) Default will check for both CLOUDFRONT and REGIONAL WAFV2.
#     REGIONAL here refers to us-east-1 ONLY. Other regions are NOT supported as of this writing.
#
# Scenarios:
#   Scenario: 1
#      Given: Rules parameter is provided
#        And: It contains a parameter key other than Scope
#       Then: Return ERROR
#   Scenario: 2
#      Given: Rules parameter is provided
#        And: There is no value specified for Scope
#       Then: Return ERROR
#   Scenario: 3
#      Given: Rules parameter is provided and CLOUDFRONT or REGIONAL or CLOUDFRONT, REGIONAL values are provided
#        And: Checks WAFV2 in the specified scope and whether it is logging.
#       Then: Return COMPLIANT with annotation if WAFV2 has logging enabled. Otherwise, return NON_COMPLIANT.
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    count = 0
    scopes = []
    # api call only supports us-east-1 region or CLOUDFRONT
    scopes = valid_rule_parameters["Scope"].split(",")
    pattern = re.compile(r'(CLOUDFRONT|REGIONAL)$')
    for scope in scopes:
        if pattern.match((scope.strip()).upper()):
            response = wafv2.list_web_acls(Scope=(scope.strip()).upper())
            if "NextMarker" in response:
                for count in range(len(response["WebACLs"])):
                    wafresults.append(response["WebACLs"][count]["ARN"])
            else:
                print("No WAF WebACL found.")
    if wafresults:
        for arnreference in wafresults:
            try:
                checklog = wafv2.get_logging_configuration(ResourceArn=arnreference)
                pattern = re.compile(r'\WLoggingConfiguration')
                if pattern.search(str(checklog)):
                    evaluations.append(build_evaluation(arnreference, 'COMPLIANT', event, annotation='This WAFV2 has logging enabled: '+str(checklog["LoggingConfiguration"]["LogDestinationConfigs"])))
            except:
                evaluations.append(build_evaluation(arnreference, 'NON_COMPLIANT', event, annotation='This WAFV2 does not have logging enabled.'))
    return evaluations
...
def evaluate_parameters(rule_parameters):
    ...
    """
    try:
        if rule_parameters["Scope"]:
            valid_rule_parameters = rule_parameters
        else:
            print("Please input a valid scope only CLOUDFRONT or REGIONAL in caps is allowed.")
        return valid_rule_parameters
    except LookupError:
        print("Please input Scope as the key.")
