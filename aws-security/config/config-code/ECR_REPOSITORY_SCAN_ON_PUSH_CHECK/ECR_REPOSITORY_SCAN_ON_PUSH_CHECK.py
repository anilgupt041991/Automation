'''
Rule Name:
  ECR_REPOSITORY_SCAN_ON_PUSH_CHECK

Description:
  Checks whether Elastic Container Registry (ECR) repositories are configured to scan new images on push. The rule is NON_COMPLIANT if the ECR repository is not configured to scan images on push.
...
Scenarios:
  Scenario: 1
     Given: No ECR repository is present
      Then: Return NOT_APPLICABLE
  Scenario: 2
     Given: There is at least 1 ECR repository
       And: ECR repository is not configured to scan images on push
      Then: Return NON_COMPLIANT with Annotation "The Amazon Elastic Container Registry repository is not configured to scan images on push."
  Scenario: 3
     Given: There is at least 1 ECR repository
       And: ECR repository is configured to scan images on push
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    ecr_client = get_client('ecr', event)

    repository_list_of_dict = get_all_repositories(ecr_client)
    result = []
    if not repository_list_of_dict:
    ...
    for repository_dict in repository_list_of_dict:
        if repository_dict['imageScanningConfiguration']['scanOnPush']:
            result.append(build_evaluation(repository_dict['repositoryArn'], 'COMPLIANT', event))
        else:
            result.append(build_evaluation(
                repository_dict['repositoryArn'],
                'NON_COMPLIANT',
                event,
...
def get_all_repositories(ecr_client):
    ...
    while True:
        if next_token:
            response_list_repositories_dict = ecr_client.describe_repositories(nextToken=next_token)
        else:
            response_list_repositories_dict = ecr_client.describe_repositories()

        result_list.extend(response_list_repositories_dict['repositories'])
        if 'nextToken' in response_list_repositories_dict:
            next_token = response_list_repositories_dict['nextToken']
        else:
            return result_list
