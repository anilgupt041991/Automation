...
'''
Description:
  Checks that both the VPN tunnels provided by AWS Site-to-Site VPN are in UP status. The rule returns NON_COMPLIANT if one or both the tunnels are reporting status as DOWN.
Trigger:
...
Scenarios:
  Scenario: 1
     Given: At least one VPN tunnel provided by AWS Site-to-Site VPN connection has status as DOWN.
      Then: Return NON_COMPLIANT with annnotation 'This AWS VPN Connection has at least one VPN tunnel down with the "statusMessage"(from configuration item)'.
  Scenario: 2
     Given: Both the tunnels provided by AWS Site-to-Site VPN Connection have status as UP.
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    raise Exception("Unknown status of the VGW")
